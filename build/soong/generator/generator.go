...
func init() {
	android.RegisterModuleType("beast_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = beastExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = beastExpandVariables(ctx, dep_file)
	...
	cmd := beastExpandVariables(ctx, String(g.properties.Cmd))
